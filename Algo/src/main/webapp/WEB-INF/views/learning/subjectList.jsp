			...
			<div id="paging">
				...
				<c:forEach var="pageNum" begin="${pm.startPage }"
					...
					<c:if test="${pm.rc.reqPage != pageNum }">
						<a	href="${pageContext.request.contextPath }/learning/list/${pageNum }${search}"
							class="off">${pageNum }</a>
					</c:if>
					<!-- 현재페이지와 요청페이지가 같으면 -->
					<c:if test="${pm.rc.reqPage == pageNum }">
				...
				<!-- 다음페이지 / 최종페이지 이동 -->
				<c:if test="${pm.next }">
					<a	href="${contextRoot }/learning/list/${pm.endPage+1}/${pm.rc.searchType}/${pm.rc.keyword}">다음</a>
					<a
